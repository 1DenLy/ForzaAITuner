...
flowchart TD
    ...
    subgraph Domain & Application Layer
        CVM -->|Обновление статуса| CSM(ConfigDataManager)
...
4. **LocalConfigRepository**
   ...
   - Несмотря на отсутствие доменной бизнес-логики, репозиторий обязан инкапсулировать базовую инфраструктурную безопасность (проверку прав доступа и защиту от Path Traversal при записи системных конфигов).
...
sequenceDiagram
    ...
    participant CSM as ConfigDataManager
...
1. Ядро приложения (`Forza Core`) запрашивает текущее состояние у `ConfigDataManager`.
...
В контексте приложения "сброс" конфигурации (`Reset`) означает **откат текущих несохраненных изменений к последнему валидному сохраненному состоянию** (через вызов `vm.get_last_valid_config()`).

**Как это работает:**
- `ConfigDataManager` всегда хранит последнюю успешно прошедшую валидацию модель.
- Если пользователь нажал `Reset`, UI запрашивает это состояние и перерисовывает форму, затирая несохраненный ввод.
- Если сохраненного конфига еще нет (первый запуск), метод вернет "дефолтные" настройки из `TuningDefaults`, так как именно они являются начальной точкой отсчета.
