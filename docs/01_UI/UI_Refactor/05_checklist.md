...
## Первичная настройка (один раз после клонирования)

```powershell
make setup-hooks
```

Это установит `pre-commit` и зарегистрирует git-хук. После этого компиляция `.ui → .py`
будет запускаться **автоматически** при каждом `git commit`, если в staged-файлах есть `.ui`.

---

## Что проверить после любой правки
