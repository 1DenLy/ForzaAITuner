...
def _make_worker(buffer: LocalBuffer, **kwargs) -> SyncWorker:
    """Factory that always provides a valid serializer so tests stay focused."""
    return SyncWorker(
        buffer=buffer,
        api_url="http://dummy",
        serializer=lambda batch: [{"v": item} for item in batch],
        **kwargs,
    )
...
async def test_worker_successful_send():
    ...
    worker = _make_worker(buffer, batch_size=10, interval_sec=0.01)
...
async def test_worker_stop_force_flushes():
    """stop() должен выполнить force-flush остатков буфера.

    Соблюдаем реальный жизненный цикл объекта: start() → mock → stop().
    Это гарантирует, что сессия инициализирована и assert внутри _send_batch
    не маскирует настоящие ошибки.
    """
    buffer = LocalBuffer()
    for i in range(1, 6):
        buffer.put_nowait(i)
    ...
    # Запускаем воркер — сессия создаётся, task стартует
    await worker.start()

    # Подменяем _send_batch ПОСЛЕ start(), чтобы не трогать реальную сеть,
    # но при этом assert self._session is not None уже пройден естественно
