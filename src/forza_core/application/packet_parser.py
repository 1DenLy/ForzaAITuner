...
class PacketParser:
    ...
    """
    # Little-endian format string for Forza Data Out (324 bytes)
    ...
    # Validation of the format string length vs 311 vs 324
    # The standard 'Dash' is 311 bytes.
    # If the user insists on 324, there are 13 bytes padding at the end.
    
    @classmethod
    def parse(cls, data: bytes) -> TelemetryPacket:
        if len(data) != 324:
            raise ValueError(f"Invalid packet size: {len(data)}, expected 324")

        # Unpack the first 311 bytes which contain the known data
        # We ignore the last 13 bytes of padding
