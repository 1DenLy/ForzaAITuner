...
# Forza Horizon 4 / "Sled+" format — 324 bytes, little-endian.
# FH4 = FM7 (311 bytes) + 1 undocumented u32 placeholder + 3 extra floats (12 bytes) = 324 bytes
# The extra 13 bytes are appended AFTER the FM7 layout.
_FH4_FORMAT = (
    '<'
    'i'    # s32  IsRaceOn
    'I'    # u32  TimestampMS
    ...
    # FH4 extensions (+13 bytes)
    'I'    # u32  HorizonPlaceholder (undocumented gap)
    'fff'  # f32  TireTempFade FL/FR/RL  — only 3 in FH4, 4th is in FH5
)

_FH4_EXPECTED_SIZE = 324


class Fh4Decoder:
    """
    IPacketDecoder implementation for Forza Horizon 4.
    Handles the "Sled+" packet format: 324 bytes, little-endian.

    FH4 = FM7 layout (311 bytes) + 4 bytes (undocumented placeholder) +
          12 bytes (3x TireTempFade floats) = 324 bytes (padded to 328 in some builds,
          but recvfrom(512) will return exactly what the game sends).

    Registered in PacketDecoderFactory with size key = 324.
    """

    def decode(self, data: bytes, received_at: datetime) -> RawTelemetry:
        """
        Unpacks 324-byte FH4 binary payload into RawTelemetry primitives.

        Raises:
            struct.error — if data is corrupted or truncated.
        """
        fields = struct.unpack_from(_FH4_FORMAT, data)
        return RawTelemetry(
            ...
            # FH4 extended fields
            HorizonPlaceholder=fields[85],
            TireTempFadeFL=fields[86],
            TireTempFadeFR=fields[87],
            TireTempFadeRL=fields[88],
        )
