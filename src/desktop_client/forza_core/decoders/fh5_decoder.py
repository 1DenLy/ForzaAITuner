...
# Forza Horizon 5 format — 331 bytes, little-endian.
# FH5 = FH4 (324 bytes) + 1 additional TireTempFade float (4 bytes) + 3 padding bytes = 331 bytes
# The 4th TireTempFadeRR field completes the set started in FH4.
_FH5_FORMAT = (
    '<'
    'i'    # s32  IsRaceOn
    'I'    # u32  TimestampMS
    ...
    # FH4/FH5 extensions
    'I'    # u32  HorizonPlaceholder
    'ffff' # f32  TireTempFade FL/FR/RL/RR (all 4 in FH5)
)

_FH5_EXPECTED_SIZE = 331


class Fh5Decoder:
    """
    IPacketDecoder implementation for Forza Horizon 5.
    Handles the FH5 packet format: 331 bytes, little-endian.

    FH5 = FH4 layout (324 bytes) + 1 additional TireTempFade float (4 bytes)
          + 3 alignment bytes = 331 bytes total.

    Registered in PacketDecoderFactory with size key = 331.
    """

    def decode(self, data: bytes, received_at: datetime) -> RawTelemetry:
        """
        Unpacks 331-byte FH5 binary payload into RawTelemetry primitives.

        Raises:
            struct.error — if data is corrupted or truncated.
        """
        fields = struct.unpack_from(_FH5_FORMAT, data)
        return RawTelemetry(
